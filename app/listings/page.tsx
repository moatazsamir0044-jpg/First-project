...
function ListingsContent() {
  ...
  let filtered = mockListings.filter(l => {
    ...
    if (l.pricePerNight < minPrice || l.pricePerNight > maxPrice) return false
            ...
            <aside className="lg:w-72 shrink-0">
              <div className="bg-white rounded-[16px] border border-gray-100 p-6 space-y-6 sticky top-20">
                <div>
                  ...
                  <select value={location} onChange={e => handleLocationChange(e.target.value)} className="w-full border border-gray-200 rounded-[8px] px-3 py-2 text-sm text-[#292a2b] outline-none focus:border-[#f4603d]">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => handleGuestsChange(Math.max(1, guests - 1))} className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center hover:border-[#f4603d] transition-colors">−</button>
                    <span className="text-sm font-semibold text-[#292a2b] w-6 text-center">{guests}</span>
                    <button onClick={() => handleGuestsChange(guests + 1)} className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center hover:border-[#f4603d] transition-colors">+</button>
                  </div>
                </div>
                ...
                {(location || checkIn || checkOut || guests > 1 || selectedAmenities.length > 0) && (
                  <button onClick={() => { setLocation(''); setCheckIn(''); setCheckOut(''); setGuests(1); setSelectedAmenities([]); router.replace('/listings') }} className="w-full text-sm text-[#f4603d] hover:underline py-2">
                    Clear all filters
                  </button>
                )}
              </div>
            ...
            <div className="flex-1">
              ...
              {filtered.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-[#292a2b]/60 mb-6">Try adjusting your filters or clearing them to see all properties.</p>
                  <button onClick={() => { setLocation(''); setCheckIn(''); setCheckOut(''); setGuests(1); setSelectedAmenities([]); router.replace('/listings') }} className="bg-[#f4603d] text-white px-6 py-3 rounded-[12px] font-semibold hover:bg-[#dd4f2e] transition-colors">
                    Clear Filters
                  </button>
                </div>
              ) : (
